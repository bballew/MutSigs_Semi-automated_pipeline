...
# to run: 
# qsub -q xlong.q -j y -o /DCEG/CGF/Bioinformatics/Production/Bari/Mut_sigs_EAGLE/repeat_analysis_lung_more_samples/pipeline_dev/mutect/ run_mut_sigs_part1.sh /DCEG/CGF/Bioinformatics/Production/Bari/Mut_sigs_EAGLE/repeat_analysis_lung_more_samples/pipeline_dev/mutect/config.yaml
...
conf=$configFile snakemake -s $execDir"Snakefile_mut_sigs_part1" --cluster "qsub -V -S /bin/sh -q $queue -o $logDir -j y" --jobs $numJobs --latency-wait 300 &> $logDir"/Snakefile_mut_sigs_part1."$DATE".out"
